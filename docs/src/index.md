...
A FITS file consists of a sequence of one or more *Header and Data Units* ([`FITS_HDU`](@ref)s), each containing a [`FITS_dataobject`](@ref) block (containing one or more images) preceeded by a [`FITS_header`](@ref) of records with metainformation.
...
The [`FITS`](@ref) object `f` has been closed above but its contents was 
*autosaved* under the name `filnam` = 'matrix.fits'. To access the image data 
of `filnam` we can [`fits_read`](@ref) the [`FITS`](@ref) object 
...
[`cast_FITS_header`](@ref), [`cast_FITS_card`](@ref) and [`cast_FITS_dataobject`](@ref).
...
procedures are not very relevant because CamiFITS takes care of them when usung 
[`fits_create`](@ref) and [`fits_create`](@ref).
