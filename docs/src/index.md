...
Above, the [`FITS`](@ref) object `f` has removed from memory but its contents 
...
```
At this point the image is availble for further processing (e.g. plotting) 
in julia.
...
[`cast_FITS_dataobject`](@ref). 
These casting procedures implicitly keep track of comformance to the 
[`FITS standard`](https://fits.gsfc.nasa.gov/fits_standard.html). 

For users primarily interested in image processing, the casting 
procedures may well remain out of sight as they are called internally by 
